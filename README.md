...
EduPOP의 **아이디어를 제안하고 서비스 기획부터 핵심 기능 설계·구현까지** 담당했습니다.

### 서비스 기획
학원 운영 경험을 바탕으로 시험 결과가 단순한 점수 확인에서 끝나지 않고 다음 수업과 학생 관리에 활용될 수 있도록 서비스 흐름을 기획했습니다.

**시험·채점 → 성적 분석 → 취약 영역 확인 → 다음 수업 활용**

### 3분 대시보드
교사가 시험 결과를 빠르게 확인하고 다음 수업에서 확인할 학생과 취약 영역을 파악할 수 있도록 기획·구현했습니다.
...
> ‘3분’은 실제 처리시간을 측정한 성능 지표가 아니라, 교사가 짧은 시간 안에 수업에 필요한 정보를 확인할 수 있도록 한 **사용 목표**입니다.

### 개인·기간별 성적 분석
학생 개인의 시험 결과를 영역별로 확인하고 성취도와 취약 영역을 파악할 수 있도록 기획·구현했습니다.

- 시험별 성적 확인
- 영역별 성취도 시각화
- 기간별 성적 추이
- 3분 대시보드에서 개인 분석으로 연결

### 반 생성 및 학생·강사 배정
학원 운영의 기본 단위인 반과 학생·강사의 관계를 관리하는 기능을 담당했습니다.

- 반 생성
- 강사 배정
- 학생 배정
- 중복 배정 방지
- 기존 시험·답안 기록을 고려한 상태 관리

### 분석 데이터 재사용
제가 구현한 성적 분석 결과를 다른 팀원의 **월간 학생 성적표**에서도 활용할 수 있도록 공통 응답 구조와 메서드를 구성했습니다.

**성적 데이터 → 공통 분석 결과 → 개인 분석 / 월간 성적표**로 이어지도록 데이터와 로직을 재사용했습니다.
